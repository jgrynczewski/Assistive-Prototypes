...
class ask( wx.Frame ):
	...
	def initializeParameters(self):
		...
		if self.switchSound.lower( ) != 'off' or self.pressSound.lower( ) != 'off':
			...
			self.takSound = mixer.Sound( self.pathToAP + '/sounds/tak.ogg' )
			self.nieSound = mixer.Sound( self.pathToAP + '/sounds/nie.ogg' )
	...
	#-------------------------------------------------------------------------	
		...
		self.functionButtonPath = [ wx.BitmapFromImage( wx.ImageFromStream( open(self.pathToAP + 'icons/back.png', 'rb' ) ) ), wx.BitmapFromImage( wx.ImageFromStream( open(self.pathToAP + 'icons/modules/tak.png', 'rb' ) ) ), wx.BitmapFromImage( wx.ImageFromStream(open(self.pathToAP + 'icons/modules/nie.png', 'rb' ) ) ) ]
	...
	#-------------------------------------------------------------------------
        def onMouseOver(self, event):
            ...
            if self.restFlag == False:
                    if self.pauseAfterChoice == True: #To jeżeli uzytkownik najechal na przycisk w trybie pauseAfterChoice (patrz timerUpdate)
                            self.pauseAfterChoice = False
                            self.iter2 = 0
                    ...
                    if self.b.mode == 'pointing':
                            if self.b.Name == 'yes':
                                    self.b.SetBackgroundColour(wx.Colour(0,150,0, self.b.GetBackgroundColour().Alpha()))
                            if self.b.Name == 'no':
                                    self.b.SetBackgroundColour(wx.Colour(150,0,0, self.b.GetBackgroundColour().Alpha()))
                            if self.b.Name == 'back':
	...
	def timerUpdate(self , event):
                ...
                if self.restFlag == True:
                    self.iter1+=1
                    if self.iter1 == 30: #Wstrzymanie na wyborze. Długość podświetlenia.
                        items = self.subSizers[ 0 ].GetChildren( )
                        for item in items:
                                    b = item.GetWindow( )
                                    if b.mode == 'confirmation':
                                            bSize = b.GetSize()
                                            ...
                                            # if self.b.Name == 'no':
                                            #         self.b.SetBackgroundColour(wx.Colour(150,0,0, self.b.GetBackgroundColour().Alpha()))
                                            b.SetBackgroundColour(self.colour)
                                            b.mode = 'pointing'

                        self.restFlag = False
                        self.pauseAfterChoice = True
                        self.iter1 = 0
                    # if self.iter > 150:
                        # if b.GetBackgroundColour() != self.colour and b.mode != 'confirmation':
                        #     if (b.Name == "yes" and b.GetBackgroundColour().Green() > 250 or b.Name == "no" and b.GetBackgroundColour().Red() > 250 or b.Name == "back" and b.GetBackgroundColour().Blue() > 200 ):
                        #         self.clickSound.play( )
                        #         if b.Name == 'yes':
                        #                 b.SetBackgroundColour(wx.Colour(0,200,0, b.GetBackgroundColour().Alpha()))
                        #         if b.Name == 'no':
                        #                 b.SetBackgroundColour(wx.Colour(200,0,0, b.GetBackgroundColour().Alpha()))
                
                elif self.control == 'eye':
                        if self.pauseAfterChoice == True:
                                if self.iter2 == 29: #Długosc przerwy po wyborze, kiedy nic nie jest podswietone i nic nie jest wybierane (najechanie na przycisk inny niż aktualny automatycznie wyrzuca z trybu pauseAfterChoice (patrz metoda OnMouseOver)) - CALA LOGIKA TYLKO DLATEGO, ZE DO PODSWIETLENIA WYKORZYSTYWANA JEST WBUDOWANY EVENT ONMOUSEOVER, KTORY JEST TRIGGEROWANY W MOMENCIE NAJECHANIA NA PRZYCISK - CZYLI BEZ TEGO, KIEDY ZOSTAL WYKONANY WYBOR NIC BY SIE NIE ZAZNACZALO DOPOKI UZYTKOWNIK NIE SPOJRZALBY NA INNY NIz WCZESNIEJ WYBRANY PRZYCISK. 
                                        self.mousePositionAtTheEnd = self.mouseCursor.position()
                                        self.iter2 += 1
                                if self.iter2 == 30:
                                        self.mouseCursor.move( *self.mousePosition )			
                                        self.iter2 += 1
                                if self.iter2 == 31:
                                        self.mouseCursor.move( *self.mousePositionAtTheEnd )
                                        self.pauseAfterChoice = False
                                        self.iter2 = 0
                                else:
                                        self.iter2 += 1
                                    ...
                                    if (b.Name == "yes" and b.GetBackgroundColour().Green() > 250 or b.Name == "no" and b.GetBackgroundColour().Red() > 250 or b.Name == "back" and b.GetBackgroundColour().Blue() > 200 ):
                                        ...
                                        for button in items:
                                            ...
                                            if b2!=b and b2.mode == 'confirmation':
                                                ...
                                                b2.mode = 'pointing'
                                                b2Size = b2.GetSize()
                                                b2.SetSize((b2Size[0]/1.05, b2Size[1]/1.05))
                                                b2Position = b2.GetPosition()
                                                b2.SetPosition((b2Position[0]-(b2Size[0]/1.05-b2Size[0])/2., b2Position[1]-(b2Size[1]/1.05-b2Size[1])/2.))
                                        ...
                                        #         b2.mode = 'pointing'
                                        #         bSize = b.GetSize()
                                        #         b.SetSize((bSize[0]*0.95, bSize[1]*0.95))
                                        #         bPosition = b.GetPosition()
                                        #         b.SetPosition((bPosition[0]-(bSize[0]*0.95-bSize[0])/2., bPosition[1]-(bSize[1]*0.95-bSize[1])/2.))
                                    else:
                                        if b.Name == 'yes':                                    
                                                b.SetBackgroundColour(wx.Colour(b.GetBackgroundColour().Red(),b.GetBackgroundColour().Green()+3,b.GetBackgroundColour().Blue(), b.GetBackgroundColour().Alpha()))
                                        elif b.Name == 'no':                                                    
                                                b.SetBackgroundColour(wx.Colour(b.GetBackgroundColour().Red()+3,b.GetBackgroundColour().Green(),b.GetBackgroundColour().Blue(), b.GetBackgroundColour().Alpha()))
                                        elif b.Name == 'back':                                                    
                                                b.SetBackgroundColour(wx.Colour(b.GetBackgroundColour().Red(),b.GetBackgroundColour().Green(),b.GetBackgroundColour().Blue(), b.GetBackgroundColour().Alpha()))
