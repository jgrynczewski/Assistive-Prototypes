...
class song( wx.Frame ):
	def __init__(self, parent, id):
		...
		wx.Frame.__init__( self , parent , id, 'ATsong' )
			...
			if self.pressFlag == False:
				...
				if self.label == 'back':
					...
					try:                                    					
						...
						songPilot.pilot( self, id=2 ).Show( True )
						os.system( 'smplayer -no-fullscreen  %s &' % choicePath.replace( ' ', r'\ ' ).replace( '(', r'\(' ).replace( ')', r'\)' ).replace( '[', r'\[' ).replace( ']', r'\]').replace( '&', r'\&' ) )

						os.system( 'sleep 2 && wid=`xdotool search --onlyvisible --name SMPlayer` && xdotool windowfocus $wid && xdotool key --window $wid f && sleep 1 && wid=`xdotool search --onlyvisible --name songPilot` && xdotool windowactivate $wid &' )
			...
			if self.numberOfPresses == 1:
				...
				if self.flag == 'rest':
					...
					try:                                    					
						...
						songPilot.pilot( self, id=2 ).Show( True )
                                                # print('smplayer -no-fullscreen  %s &' % choicePath.replace( ' ', r'\ ' ).replace( '(', r'\(' ).replace( ')', r'\)' ).replace( '[', r'\[' ).replace( ']', r'\]').replace( '&', r'\&' ))
                                                # exit()
						os.system( 'smplayer -no-fullscreen  %s &' % choicePath.replace( ' ', r'\ ' ).replace( '(', r'\(' ).replace( ')', r'\)' ).replace( '[', r'\[' ).replace( ']', r'\]').replace( '&', r'\&' ) )
