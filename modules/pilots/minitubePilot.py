		...
		self.numberOfEmptyIteration = 0
			...
			if self.flag == 'row':
				...
				if self.rowIteration == 0 or self.rowIteration == 1:
					...
					self.rowIteration = 0
					os.system("wid=`wmctrl -l | awk '/Minitube/ {print $1}'` && xdotool windowfocus $wid && xdotool key --window $wid F11 && wid=`wmctrl -l | awk '/Pilot/ {print $1}'` && xdotool windowactivate $wid' #&& xdotool keydown alt key Tab; sleep 0.2; xdotool keyup alt'  #&& wid2=`wmctrl -l | awk '/Pilot/ {print $1}'` && sleep 0.5 && wmctrl -a Pilot ")
