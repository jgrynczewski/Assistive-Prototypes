...
class speller( wx.Frame ):
	...
	def createGui(self):
		...
		for item in ( self.labels[ 0 ][ self.specialButtonsMarker ], ):
			...
			b.SetFont( wx.Font( self.tableFontSize, eval(self.tableFont), wx.FONTWEIGHT_NORMAL,  False ) )
			b.SetBezelWidth( 3 )
			b.SetBackgroundColour( self.backgroundColour )
			b.SetForegroundColour( self.textColour )
			b.Bind( event, self.onPress )
...
class pilot(wx.Frame):
	def __init__(self, parent, id):
            ...
            self.winHeight = root.winfo_screenheight() - 20
            root.destroy()
	...
	def initializeParameters(self):
            ...
            # alsaaudio.Mixer( control = 'Master' ).setvolume( 75, 0 )
	    ...
	    self.pageFlipSounds = glob.glob( '../sounds/page_flip/*' )
	...
	#-------------------------------------------------------------------------	
        def initializeBitmaps(self):
            ...
            radioLogoPaths = glob.glob('../icons/pilots/minitubePilot/*') #labelFiles
	...
	#-------------------------------------------------------------------------
		...
		self.numberOfEmptyIteration = 0
			...
			if self.flag == 'row':
				...
				selectedButton.SetBackgroundColour( self.selectionColor )
...
########################
				...
				# elif self.radios[self.position][0] == 'undo':
					...
					# self.menu.Show()
                                ...
                                selectedButton.SetBackgroundColour( self.backgroundColor )
				self.flag = 'row'
                                self.panelIteration = 0
				self.rowIteration = 0
				self.colIteration = 0
